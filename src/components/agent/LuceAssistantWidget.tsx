...
import { askLuceAssistant } from "@/lib/luce-agent-api";
...
const STORAGE_KEY = "luce-assistant-messages";
...
export function LuceAssistantWidget() {
  ...
  const submit = async (rawMessage: string) => {
    ...
    try {
      const result = await askLuceAssistant(message, controller.signal);
      setMessages((current) =>
        [
          ...current,
